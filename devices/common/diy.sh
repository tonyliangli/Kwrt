...
sed -i '$a src-git kiddin9 https://github.com/tonyliangli/kwrt-packages.git;main' feeds.conf.default
...
status=$(curl -H "Authorization: token $REPO_TOKEN" -s "https://api.github.com/repos/tonyliangli/kwrt-packages/actions/runs" | jq -r '.workflow_runs[0].status')
echo "$status"
